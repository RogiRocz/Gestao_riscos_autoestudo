    ...
    <v-layout row>
      <v-flex grow pa-1>
        <v-card>
          ...
          <!-- Gráfico Histórico de Ocorrências-->
           <v-toolbar card color="white">
            <h4>
              <b>Ocorrências</b>
            </h4>              
            ...
            <v-layout row wrap>
              <v-flex xs3 >
                ...
                >
                  ...
                  <v-date-picker v-model="dataInicio" @change="filtrarOcorrencias(), remove(), load()" @input="dateInicio = false" locale="pt-br"></v-date-picker>
                </v-menu>
              </v-flex>
              ...
              <v-flex xs3>
                ...
                >
                  ...
                  <v-date-picker v-model="dataFim" @change="filtrarOcorrencias(), remove(), load()" @input="dateFim = false" locale="pt-br"></v-date-picker>
                </v-menu>
              </v-flex>
              <v-spacer></v-spacer>
               <v-flex sm4 xs12 class="text-xs-right">
                <v-select
                    ...
                    :items="unidades"
                    label="Unidade"
                    @change="filtrarOcorrencias(), remove(), load()"
                ></v-select>
                </v-flex>    
            </v-layout>     
          </v-toolbar>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
              <v-btn color="primary" small @click="limparFiltro()">Limpar filtros</v-btn>
              </v-card-actions>
          </v-flex>
          <div>
      ...
      <v-flex d-flex xs12 sm6 md3 style="margin-bottom: -80px">
        <v-layout row wrap>
          <v-flex> <!-- d-flex -->
            <v-layout row wrap>
              <v-flex v-for="(c, indexC) in cards" :key="indexC" xs12> <!-- d-flex -->
  ...
  export default {
      ...
      async listar() {
        ...
        await axios
          ...
          .then(res => {
            ...
            for(let r = 0; r < this.riscos.length; r++){
              this.unidades.push(this.riscos[r].unidade.sigla);
              if(this.riscos[r].tipo == 'AMEACA'){
                  this.riscoAmeaca += 1;
              }
            }
          })
    ...
    async filtrarOcorrencias(){
      ...
      if(this.dataInicio == '' && this.dataFim == ''){
        if(this.unidadeSelecionada == "Todas"){
        await axios
        .get("/ocorrencias/")
        .then(res => {
          ...
          for(let d = 0; d < this.datas.length; d++){
            this.meses.push(moment(this.datas[d]).month());
          }
        ...
        await axios
        .get("/ocorrencias/unidade/" +this.unidadeSelecionada)
        .then(res => {
          this.ocorrencias = res.data;
          for(let o = 0; o < this.ocorrencias.length; o++){
            this.datas.push(this.ocorrencias[o].data);    
      ...
      }else if (this.dataInicio != '' && this.dataFim != '' && (this.unidadeSelecionada == 'Todas' || this.unidadeSelecionada == '')){
          const dataInicial = this.dataInicio, dataFinal = this.dataFim;
          await axios.get("/ocorrencias/data", {
          params: {
            dataInicial,
            dataFinal
          }
        ...
        .catch(error => console.log(error));
         ...
         const dataInicial = this.dataInicio, dataFinal = this.dataFim, siglaUnidade = this.unidadeSelecionada;
          await axios.get("/ocorrencias/filtro", {
          params: {
            dataInicial,
            dataFinal,
            siglaUnidade
